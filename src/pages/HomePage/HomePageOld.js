...
class HomePage extends React.Component {
    ...
    addCoordinate = (xValue, yValue) => {
        console.log('addCoordinate: x,y ', xValue, yValue)
        let coord = {
            x:xValue,
            y:yValue 
        }
        ...
        this.setState( prevState => ({
            data: [...prevState.data, coord]
        }));
    }


    deleteCoordinate = (xValue, yValue) => {
        var xTmp = parseInt(xValue);
        var yTmp = parseInt(yValue);
        var dtaTmp = this.state.data
        console.log('state.data', this.state.data)
        const coordIndex = dtaTmp.findIndex(coord => coord.x === xTmp && coord.y === yTmp);
        if (coordIndex > -1) {
            this.state.data.splice(coordIndex, 1)
            console.log('after splice', this.state.data)
            dtaTmp = this.state.data
